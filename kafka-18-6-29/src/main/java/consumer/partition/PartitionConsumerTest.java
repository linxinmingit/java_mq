...
public class PartitionConsumerTest {
	public static void main(String args[]) {
		...
		String hosts="192.168.1.138,192.168.1.175,192.168.1.180"; // kafka ip
		String[] hostArr = hosts.split(",");
		for(int index = 0;index < hostArr.length;index++){
			seeds.add(hostArr[index].trim());
		}
		
		int port = 9092; // kafka 端口
		 
        int partLen = Integer.parseInt("3");
		for(int index=0;index < partLen;index++){
	        try {
	        	System.out.println();
	        	System.out.println("读取 - " + index + " - 分区的数据");
	        	System.out.println();
	            example.run(maxReads, topic, index, seeds, port); // 从0开始读取,一直读到你的分区数量结束
	        } catch (Exception e) {
